...
COALESCE(R.callamount,0)) > 3275.07 then 'High' else 'Low' end AS TotalBetGroup,
...
and G.turnplayeractiontype_id in 
(select actiontype_id from actiontypes
where trim(actionstring) not like '%F%'
and  trim(actionstring) not like '%R%'
and  trim(actionstring) not like 'B%'
...
and G.riverplayeractiontype_id in 
(select actiontype_id from actiontypes
where trim(actionstring) not like '%F%'
and  trim(actionstring) not like '%R%'
and  trim(actionstring) not like 'B%'
...
COALESCE(T.callamount,0)+
COALESCE(R.betamount,0)+
COALESCE(R.callamount,0)) > 3275.07 then 'High' else 'Low' end

order by G.positiontype_id, TotalBetGroup
